...
function onCreatePost()
    ...
    for i = 1, 3 do
        ...
        makeAnimatedLuaSprite(tag, 'noteComboNumbers', x - 30 + i * 125,
                              y + 110 - i * 50)
...
function onUpdate()
    if lastMustHit ~= mustHitSection then
        ...
        if not lastMustHit and noteHits > 6 and
			...
			objectPlayAnimation("noteCombo","appear",true)
    ...
    if getProperty('noteCombo.animation.finished') then
		...
		--debugPrint("Anim Ended. CurAnim Name: ",ateUrFrame)
        if ateUrFrame == 'appear' then
			--debugPrint("If Statement works")
            --playAnim('disappear',true)
			objectPlayAnimation("noteCombo","disappear",true)
            for i = 1, 3 do
				--debugPrint("looping. Interation "..i)
                local tag = 'noteComboN' .. i
                local num = string.sub(seperatedHits, i, i)
                if num ~= '' and num ~= ' ' then
        ...
        elseif ateUrFrame == 'disappear' then
            setProperty('noteCombo.visible', false)
            setProperty('noteCombo.active', false)
            -- not the same frames length, but shut the f*** up
            for i = 1, 3 do
                local tag = 'noteComboN' .. i
                local num = string.sub(seperatedHits, i, i)
                if num ~= '' and num ~= ' ' then
