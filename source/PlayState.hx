...
class PlayState extends MusicBeatState
{
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['weekD_nomiss', 'weekTD_nomiss']);
			if(achieve != null) {
		...
		if(note.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
			...
			if (SONG.notes[curSection] != null)
			{
				if (SONG.notes[curSection].altAnim || note.noteType == 'Alt Animation') {
					altAnim = '-alt';
				} else if (note.noteType == 'Table Nofeet'){
				altAnim = '-NF';
				} else if (note.noteType == 'Table WT'){
					altAnim = '-WT';
				} 
			}
	...
	private function checkForAchievement(achievesToCheck:Array<String> = null):String
	{
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				{
					case 'weekD_nomiss' | 'weekTD_nomiss':
						if(isStoryMode && campaignMisses + songMisses < 1 && CoolUtil.difficultyString() == 'HARD' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
						{
							...
							{
								case 'weekD':
									if(achievementName == 'weekD_nomiss') unlock = true;
								case 'weekTD':
									if(achievementName == 'weekTD_nomiss') unlock = true;
							}
						}
					case 'ur_bad':
